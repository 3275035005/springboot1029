...
public class UserController {
    ...
    public R info(String token){
        ...
        if("1".equals(user.getRole())){
            ...
            user.setRoles(new String[]{"teacher"});
    ...
    public R getHome(){
        ...
        map.put("studentNumber",userNumber);

        int teacherNumber = userService.count(new QueryWrapper<User>().eq("role", "2"));
        map.put("teacherNumber",userNumber);

        int laboratoryNumber = laboratoryAppointmentService.count(null);
        map.put("laboratoryNumber",laboratoryNumber);

        int accomplishNumber =  laboratoryAppointmentService.count(new QueryWrapper<LaboratoryAppointment>().eq("status", "2"));
